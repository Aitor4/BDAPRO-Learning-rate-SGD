...
class AdagradGradientDescent private[spark] (private var gradient: Gradient, private var updater: AdagradUpdater)
  ...
  def optimize(data: RDD[(Double, Vector)], initialWeights: Vector): Vector = {
    val (weights, _) = AdagradGradientDescent.runMiniBatchSGD(
      data,
      gradient,
      updater,
      stepSize,
...
object AdagradGradientDescent extends Logging {
  ...
  def runMiniBatchSGD(
    ...
    AdagradGradientDescent.runMiniBatchSGD(data, gradient, updater, stepSize, numIterations,
      regParam, miniBatchFraction, initialWeights, smoothingTerm, 0.001)
